...
class Node:
    ...
    def __init__(self, name,id, n, k, data_size, storage_size, upload_speed, download_speed, average_uptime, average_downtime, average_lifetime, average_recover_time, arrival_time,selfish=False):
        self.name = name
        self.id = id
        self.n = n
        self.k = k
    ...
    selfish: bool  # whether the node is selfish
...
def main():
    ...
    parser.add_argument("--config", default="p2p-extension.cfg", help="configuration file")
    ...
    parsing_functions = [
        ...
        ('arrival_time', parse_timespan), ('selfish', bool)
    ]
    ...
    for k in range(0,10):
        ...
        for l in range(0,random_number):
            countte[l] = random.randint(0, 9)
        ...
        for r in range(10,120,10):
            ...
            print(f"Number of selfish nodes with varient lifetime: {counter_var_selfish}")
            ...
            """
            print(f"number of blockes that are backed up {sim.backed_up_blocks_count}")
            print(f"number of blockes that are restored {sim.restored_blocks_count}")
        

            
            
            # Calculate the ratio between the sum of even elements and r
            ...
            counter+=1
        count_restore[k][0]=sum(r_values[t][0] for t in range(11))/11
        count_restore[k][1]=sum(r_values[m][1] for m in range(11))/11
        count_backup[k][0]=sum(b_values[t][0] for t in range(11))/11
        count_backup[k][1]=sum(b_values[m][1] for m in range(11))/11
        # Plot the ratio
    
    plt.plot(range(10, 120, 10), [r[0] for r in r_values], linestyle='-', color='blue', label='Var Restored Blocks')
    plt.plot(range(10, 120, 10), [r[1] for r in r_values], linestyle='-', color='purple', label='NonVar Restored Blocks')    
    plt.plot(range(10, 120, 10), [b[0] for b in b_values], linestyle='--', color='red', label='Var Backed Up Blocks')
    plt.plot(range(10, 120, 10), [b[1] for b in b_values], linestyle='--', color='black', label='Non Var Backed Up Blocks')
    plt.plot(range(10, 120, 10), r_vs_b, linestyle=':', color='green', label='ratio')
    plt.xlabel('50% of nodes have a varient lifetime ')
    plt.ylabel('ratio of restored blocks that have varient life time with the rest of the nodes')
    plt.title(f"{100/vr}% varient")
    plt.legend()
    plt.grid(True)
    plt.ylim(0, 1.1)
    plt.xlim(0, 120)
