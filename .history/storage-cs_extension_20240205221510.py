...
def main():
    ...
    for _ in range(0,10):
        ...
        for r in range(10,120,10):
            ...
            if sum(sim.restored_blocks_count) != 0:
        ...
        print(f"{counter_var_selfish} selfish nodes were simulated And Have Varient Lifetime.")
    ...
    plt.xlabel('20% of nodes have a varient lifetime ')
    plt.ylabel('ratio of restored blocks that have varient life time with the rest of the nodes')
    plt.title(f"20% varient With {random_number} Selfish_nodes ")
    plt.legend()
    plt.grid(True)
    plt.ylim(0, 1.1)
    plt.xlim(0, 120)
