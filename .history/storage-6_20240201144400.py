...
class Backup(Simulation):
    ...
    def count_backed_up_blocks(self):
        ...
        for node in self.nodes:
            count += sum(peer is not None for peer in node.backed_up_blocks)
        return count
...
def main():
    ...
    print(f"number of blockes that are backed up {sum(sum(peer is not None for peer in node.backed_up_blocks) for node in nodes)}")
