...
def main():
    ...
    for r in range(10,120,10):
        ...
        even_sum_ratio_r = sum(sim.restored_blocks_count[i] for i in range(len(sim.restored_blocks_count)) if i % 2 == 0)
        odd_sum_ratio_r = sum(sim.restored_blocks_count[i] for i in range(len(sim.restored_blocks_count)) if i % 2 != 0)
        ratio_r = even_sum_ratio_r / odd_sum_ratio_r
        r_values[counter_r] = ratio_r
        counter_r +=1
        even_sum_ratio_b = sum(sim.backed_up_blocks_count[i] for i in range(len(sim.backed_up_blocks_count)) if i % 2 == 0)
        odd_sum_ratio_b = sum(sim.backed_up_blocks_count[i] for i in range(len(sim.backed_up_blocks_count)) if i % 2 != 0)
        ratio_b = odd_sum_ratio_b / even_sum_ratio_b
        b_values[counter_b] = ratio_b
        counter_b +=1
        r_vs_b[counter_r]=sum(sim.restored_blocks_count)/ sum(sim.backed_up_blocks_count)
        # Plot the ratio
    print(b_values)
    ...
    plt.plot(range(10, 120, 10), r_vs_b, linestyle='--', color='green', label='Backed Up Blocks')
