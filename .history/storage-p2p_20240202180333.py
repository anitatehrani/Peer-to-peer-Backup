...
class Backup(Simulation):
    ...
    def schedule_transfer(self, uploader: 'Node', downloader: 'Node', block_id: int, restore: bool):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(self._process_transfer, delay, event)

            # Save the future in the uploader and downloader so that it can be canceled if needed
            uploader.current_upload = downloader.current_download = future
 
        self.schedule(delay, future)
        uploader.current_upload = downloader.current_download = event
    def _process_transfer(self, delay, event):
        """Process the transfer after the specified delay."""
