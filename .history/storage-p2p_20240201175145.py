...
class Backup(Simulation):
    ...
    #def count_backed_up_blocks(self):
        #global count
        #for node in self.nodes:
            #count += len(node.backed_up_blocks)
...
class BlockRestoreComplete(TransferComplete):
    def update_block_state(self):
        ...
        if sum(owner.local_blocks) == owner.k:  # we have exactly k local blocks, we have all of them then
            ...
            # Iterate over the values of average_lifetime in the config file
            for node_class in config.sections():
                class_config = config[node_class]
                average_lifetime_values = parse_timespan(class_config['average_lifetime'])
                for average_lifetime in average_lifetime_values:
                    # Set the average_lifetime value in the node configuration
                    cfg = [parse(class_config[name]) if name == 'average_lifetime' else parse(class_config[name]) for name, parse in parsing_functions]
                    cfg[parsing_functions.index(('average_lifetime', parse_timespan))] = average_lifetime
                    ...
                    print(f"Average download time: {sum(node.average_downtime for node in nodes)/len(nodes)}")
                    print(f"Average upload time: {sum(node.average_uptime for node in nodes)/len(nodes)}" + "\n\n")
                    with open("results.txt", "a+") as f:
                        f.write(f"Average download time: {sum(node.average_downtime for node in nodes)/len(nodes)}" + "\n")
                        f.write(f"Average upload time: {sum(node.average_uptime for node in nodes)/len(nodes)}" + "\n")
                    print(f"Average time spent online: {sum(node.average_uptime for node in nodes)/len(nodes)}")
                    print(f"Average time spent offline: {sum(node.average_downtime for node in nodes)/len(nodes)}")
                    print(f"Average time before a crash and data loss: {sum(node.average_lifetime for node in nodes)/len(nodes)}")
                    print(f"Average time after a data loss: {sum(node.average_recover_time for node in nodes)/len(nodes)}")
                    print(f"Number of nodes that failed: {sum(node.failed for node in nodes)}")
                    print(f"Number of nodes that are online: {sum(node.online for node in nodes)}")
                    print(f"Number of nodes that are offline: {sum(not node.online for node in nodes)}")
                    print(f"Number of nodes that are under repairs: {sum(node.failed for node in nodes)}")
                    print(f"Number of nodes that are not under repairs: {sum(not node.failed for node in nodes)}")
                    print(f"Number of nodes that have all their blocks: {sum(sum(node.local_blocks) == node.k for node in nodes)}")
                    print(f"Number of nodes that have not all their blocks: {sum(sum(node.local_blocks) != node.k for node in nodes)}")
                    print(f"Average download speed: {sum(node.download_speed for node in nodes)/len(nodes)}")
                    print(f"Average upload speed: {sum(node.upload_speed for node in nodes)/len(nodes)}")
                    print(f"Average storage size: {sum(node.storage_size for node in nodes)/len(nodes)}")
                    print(f"Number of nodes that backed up all their blocks: {sum(sum(peer is not None for peer in node.backed_up_blocks) == node.n for node in nodes)}")
                    print(f"Number of blocks that are backed up: {count}")
        # the `callable(p1, p2, *args)` idiom is equivalent to `callable(p1, p2, args[0], args[1], ...)
        nodes.extend(Node(f"{node_class}-{i}", *cfg) for i in range(class_config.getint('number')))
    sim = Backup(nodes)
    sim.run(parse_timespan(args.max_t))
    sim.log_info(f"Simulation over")
    print(f"avrage download time {sum(node.average_downtime for node in nodes)/len(nodes)}")
    print(f"avrgae upload time {sum(node.average_uptime for node in nodes)/len(nodes)}" + "\n\n")
    with open("results.txt", "a+") as f:
        f.write(f"avrage download time {sum(node.average_downtime for node in nodes)/len(nodes)}" + "\n")
        f.write(f"avrgae upload time {sum(node.average_uptime for node in nodes)/len(nodes)}" + "\n")
    print(f"avrage of time spent online {sum(node.average_uptime for node in nodes)/len(nodes)}")
    print(f"avrage of time spent offline {sum(node.average_downtime for node in nodes)/len(nodes)}")
    print(f"the avrage of time before a crash and data loss {sum(node.average_lifetime for node in nodes)/len(nodes)}")
    print(f"the avrage of time after a data loss {sum(node.average_recover_time for node in nodes)/len(nodes)}")
    print(f"number of nodes that failed {sum(node.failed for node in nodes)}")
    print(f"number of nodes that are online {sum(node.online for node in nodes)}")
    print(f"number of nodes that are offline {sum(not node.online for node in nodes)}")
    print(f"number of nodes that are under repairs {sum(node.failed for node in nodes)}")
    ...
    print(f"number of nodes that have all their blocks {sum(sum(node.local_blocks) == node.k for node in nodes)}")
    print(f"number of nodes that have not all their blocks {sum(sum(node.local_blocks) != node.k for node in nodes)}")
    print(f"avrage download speed {sum(node.download_speed for node in nodes)/len(nodes)}")
    print(f"avrage upload speed {sum(node.upload_speed for node in nodes)/len(nodes)}")
    print(f"avrage storage size {sum(node.storage_size for node in nodes)/len(nodes)}")
    print(f"number of nodes that backed up all their blocks {sum(sum(peer is not None for peer in node.backed_up_blocks) == node.n for node in nodes)}")
    
    print(f"number of blockes that are backed up {count}")
