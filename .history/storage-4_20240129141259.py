...
class Node:
    ...
    def schedule_next_download(self, sim: Backup):
            ...
            for block_id, (held_locally, peer) in enumerate(zip(self.local_blocks, self.backed_up_blocks)):
                if not held_locally and peer is not None and peer.current_upload is None and peer.local_blocks[block_id] is None:
                    self.current_download = TransferComplete(self, peer, block_id, self.download_speed)
                    sim.schedule_event(self.current_download)
            ...
            for peer in sim.nodes:
                if (peer is not self and peer.online and peer.current_upload is None and peer not in self.remote_blocks_held
                    ...
                    if block_id is not None:
                        self.current_download = TransferComplete(peer, self, block_id, peer.upload_speed)
                        sim.schedule_event(self.current_download)
                        return
