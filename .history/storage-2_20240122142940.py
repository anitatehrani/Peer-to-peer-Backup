...
from discrete_event_sim import BlockBackupComplete
from discrete_event_sim import BlockRestoreComplete
    ...
    def __post_init__(self):
        ...
        def schedule_next_upload(self, sim: Backup):
                ...
                for peer, block_id in self.remote_blocks_held.items():
                    ...
                    if not self.local_blocks[block_id] and peer is not None and peer.current_download is None:
                        self.current_upload = BlockRestoreComplete(self, peer, block_id)
                ...
                for peer in sim.nodes:
                    ...
                    if (peer is not self and peer.online and peer not in remote_owners
                            and peer.current_download is None and peer.free_space >= self.block_size):
                        self.current_upload = BlockBackupComplete(self, peer, block_id)
                        return
        ...
        def schedule_next_download(self, sim: Backup):
                ...
                for block_id, (held_locally, peer) in enumerate(zip(self.local_blocks, self.backed_up_blocks)):
                    if not held_locally and peer is not None and peer.current_upload is None:
                        self.current_download = BlockRestoreComplete(peer, self, block_id)
                ...
                for peer in sim.nodes:
                    if (peer is not self and peer.online and peer.current_upload is None and peer not in self.remote_blocks_held
                        ...
                        if block_id is not None:
                            self.current_download = BlockBackupComplete(peer, self, block_id)
                            return
