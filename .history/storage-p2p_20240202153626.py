...
def main():
    ...
    for r in range(10,100,10):
        ...
        # Get the restored_block_count and r values
        restored_block_count = sim.restored_blocks_count
        r_values = range(10, 100, 10)

        # Calculate the ratio between the sum of even elements and r
        even_sum_ratio = sum(restored_block_count[i] for i in range(len(restored_block_count)) if i % 2 == 0)
        odd_sum_ratio = sum(restored_block_count[i] for i in range(len(restored_block_count)) if i % 2 != 0)
        even_sum_ratio = even_sum_ratio / sum(restored_block_count)
        # Plot the ratio
        plt.plot(r_values, [even_sum_ratio] * len(r_values), 'r--', label='Even Sum Ratio')
        plt.xlabel('Avrage life time')
        plt.ylabel('Even Sum Ratio')
        plt.title('Restored Block Count - Even Sum Ratio')
        plt.legend()
        plt.show()
