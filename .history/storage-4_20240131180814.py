...
class Node:
    ...
    def schedule_next_upload(self, sim: Backup):
        ...
        for peer, block_id in self.remote_blocks_held.items():
            ...
            if block_id < len(self.local_blocks) and not self.local_blocks[block_id] and peer.online and not peer.local_blocks[block_id]:
...
class Online(NodeEvent):
    ...
    def process(self, sim: Backup):
        ...
        sim.schedule(exp_rv(node.average_downtime),Offline(node))
...
class BlockRestoreComplete(TransferComplete):
    def update_block_state(self):
        ...
        if sum(owner.local_blocks) == owner.k:  # we have exactly k local blocks, we have all of them then
            owner.local_blocks = [True] * owner.n  # reset local_blocks list to indicate that we don't need it anymore


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="p2o.cfg", help="configuration file")
