...
def main():
    ...
    for r in range(10,120,10):
        ...
        even_sum_ratio_r = sum(sim.restored_blocks_count[i] for i in range(len(sim.restored_blocks_count)) if i % vr == 0)
        odd_sum_ratio_r = sum(sim.restored_blocks_count[i] for i in range(len(sim.restored_blocks_count)) if i % vr != 0)
        if even_sum_ratio_r > odd_sum_ratio_r:
            ratio_r = odd_sum_ratio_r / even_sum_ratio_r
        else:
            ratio_r = even_sum_ratio_r / odd_sum_ratio_r
        r_values[counter_r] = ratio_r
        counter_r +=1
        even_sum_ratio_b = sum(sim.backed_up_blocks_count[i] for i in range(len(sim.backed_up_blocks_count)) if i % vr == 0)
        odd_sum_ratio_b = sum(sim.backed_up_blocks_count[i] for i in range(len(sim.backed_up_blocks_count)) if i % vr != 0)
        if even_sum_ratio_b > odd_sum_ratio_b:
            ratio_b = odd_sum_ratio_b / even_sum_ratio_b
        else:
            ratio_b = even_sum_ratio_b / odd_sum_ratio_r
        b_values[counter_b] = ratio_b
        counter_b +=1
        r_vs_b[counter]=sum(sim.restored_blocks_count)/ sum(sim.backed_up_blocks_count)
        counter+=1
        # Plot the ratio
    print(r_vs_b)
    plt.plot(range(10, 120, 10), r_values, linestyle='-', color='blue', label='Restored Blocks')
    plt.plot(range(10, 120, 10), b_values, linestyle='--', color='red', label='Backed Up Blocks')
