...
class Node:
    ...
    def __init__(self, name, id, n, k, data_size, storage_size, upload_speed, download_speed, average_uptime,
                 average_downtime, average_lifetime, average_recover_time, arrival_time, selfish=False):
        self.name = name
        self.id = id
        self.n = n
        self.k = k
    ...
    def schedule_next_download(self, sim: Backup):
        ...
        for peer in sim.nodes:
            if (
                    peer is not self and peer.online and peer.current_upload is None and peer not in self.remote_blocks_held.keys()
...
def main():
    ...
    for _ in range(0, 10):
        ...
        for r in range(10, 120, 10):
            ...
            var_ratio_r = sum(
                sim.restored_blocks_count[i] for i in range(len(sim.restored_blocks_count)) if i % vr == 0)
            nonvar_ratio_r = sum(
                sim.restored_blocks_count[i] for i in range(len(sim.restored_blocks_count)) if i % vr != 0)
            ...
            counter_r += 1
            var_ratio_b = sum(
                sim.backed_up_blocks_count[i] for i in range(len(sim.backed_up_blocks_count)) if i % vr == 0)
            nonvar_ratio_b = sum(
                sim.backed_up_blocks_count[i] for i in range(len(sim.backed_up_blocks_count)) if i % vr != 0)
            if sum(sim.restored_blocks_count) != 0:
    ...
    plt.plot(range(10, 120, 10), [r[0] for r in count_restore], linestyle='-', color='blue',
             label='Var Restored Blocks')
    plt.plot(range(10, 120, 10), [r[1] for r in count_restore], linestyle='-', color='purple',
             label='NonVar Restored Blocks')
    plt.plot(range(10, 120, 10), [b[0] for b in count_backup], linestyle='--', color='red',
             label='Var Backed Up Blocks')
    plt.plot(range(10, 120, 10), [b[1] for b in count_backup], linestyle='--', color='black',
             label='Non Var Backed Up Blocks')
